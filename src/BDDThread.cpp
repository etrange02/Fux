...
static BDDThread* instanceBDDThread = NULL;
static wxMutex *m_mutexGestionGlobale = new wxMutex;
...
{
    if (!instanceBDDThread)
        instanceBDDThread = new BDDThread();
    return instanceBDDThread;
}
...
void* BDDThread::Entry()
{
    ...
    while (m_continuer && !TestDestroy())
    {
        ...
        //wxMutexLocker lock(*m_mutexGestionGlobale);
        //m_mutexGestionGlobale->Lock();
