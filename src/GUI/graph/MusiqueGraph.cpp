...
void MusiqueGraph::OnPaint(wxPaintEvent&)
{
    ...
    if (!MusicManagerSwitcher::get().empty())
    {
        ...
        int largeur = MusicManagerSwitcher::get().getMusicPlayer().getCurrentTime()->GetMSecondeTot()*m_sizer_w/MusicManagerSwitcher::get().getMusicPlayer().getTotalTime()->GetMSecondeTot();
        glBegin(GL_QUADS);
        ...
        {
            int point = ScreenToClient(wxGetMousePosition()).x, tot = (point*MusicManagerSwitcher::get().getMusicPlayer().getTotalTime()->GetMSecondeTot())/(1000*m_sizer_w);
            wxString duree = wxString::Format(_T("%d:%2d/%d:%2d"), MusicManagerSwitcher::get().getMusicPlayer().getCurrentTime()->GetMinute(),
                                                                    MusicManagerSwitcher::get().getMusicPlayer().getCurrentTime()->GetSeconde(),
                                                                    MusicManagerSwitcher::get().getMusicPlayer().getTotalTime()->GetMinute(),
                                                                    MusicManagerSwitcher::get().getMusicPlayer().getTotalTime()->GetSeconde());
...
void MusiqueGraph::PlacerChanson(wxMouseEvent &event)
{
    ...
    {
        int msposition = (position.x)*(MusicManagerSwitcher::get().getMusicPlayer().getTotalTime()->GetMSecondeTot())/(m_sizer_w);
        MusicManagerSwitcher::get().getMusicPlayer().setPosition(msposition);
    }
}
...
void MusiqueGraph::OnToucheDown(wxKeyEvent &event)
{
    if (event.GetKeyCode() == 'P')
    {
        ...
        else if (MusicManagerSwitcher::get().getMusicPlayer().isPaused())
...
{
    if (MusicManagerSwitcher::get().getMusicPlayer().hasLoadedMusic())
        m_imageText->Creer(MusicManagerSwitcher::get().getMusic()->GetName(), m_largeur_tab, m_sizer_w);
    else
...
void MusiqueGraph::MouseEvents(wxMouseEvent &event)
{
    if (event.ControlDown() && event.GetWheelRotation() != 0)
    {
        if (event.GetWheelRotation() < 0)
            MusicManagerSwitcher::get().playNextMusic();
        else
            MusicManagerSwitcher::get().playPreviousMusic();
    }
