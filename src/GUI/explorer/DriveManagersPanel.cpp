...
BEGIN_EVENT_TABLE(DriveManagersPanel, wxPanel)
    ...
    ///TODO (David): Duplicate event item to make code simpler than previous version
END_EVENT_TABLE()
...
*/
    ...
    m_streamButton[STREAM_MOVE_LEFT ].Enable(false);
    m_streamButton[STREAM_COPY      ].Enable(false);
    m_streamButton[STREAM_MOVE_RIGHT].Enable(false);
    m_streamButton[STREAM_DELETE    ].Enable(false);
...
void DriveManagersPanel::fillDriversMenu()
{
    ...
    mergeAndMarkPresentElements(m_driversMenuElement, m_explorerDriveManagers.getDrivers(), &DriveManagersPanel::onMenuItemDriversLeftSelected, &DriveManagersPanel::onMenuItemDriversRightSelected);
...
void DriveManagersPanel::fillContainerFilesMenu()
{
    ...
    mergeAndMarkPresentElements(m_containerFilesMenuElement, m_explorerDriveManagers.getContainersFiles(), &DriveManagersPanel::onMenuItemContainerFilesLeftSelected, &DriveManagersPanel::onMenuItemContainerFilesRightSelected);
    fillMenu(*m_containerFilesMenu, m_containerFilesMenuElement);
}

void DriveManagersPanel::mergeAndMarkPresentElements(std::vector<MenuElement>& menuElements, std::vector<MenuElementData> menuElementData, void (DriveManagersPanel::*funcMappingLeft)(wxCommandEvent&), void (DriveManagersPanel::*funcMappingRight)(wxCommandEvent&))
{
